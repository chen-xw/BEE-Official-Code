...
def load_tokenizer(model_args: "ModelArguments") -> "TokenizerModule":
    ...
    if model_args.new_tokens is not None:
        special_vq_tokens = ["<START_OF_GEN>", "<END_OF_GEN>"] + [f"<IMG_OF_{num:04d}>" for num in range(model_args.nums_new_tokens)]
        num_added_tokens = tokenizer.add_tokens(special_vq_tokens)
        print("Number of added tokens: ", num_added_tokens)
        print(f"Added special tokens is {['<START_OF_GEN>', '<END_OF_GEN>']}, {[f'<IMG_OF_{num:04d}>' for num in range(model_args.nums_new_tokens)]}")

        vq_token_ids = tokenizer.convert_tokens_to_ids(special_vq_tokens)
        model_args.vq_token_ids = vq_token_ids
    ###### NEW #################
