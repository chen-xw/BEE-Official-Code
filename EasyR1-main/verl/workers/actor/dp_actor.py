...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        if self.rank == 0:
            ...
            print(f"Compute log probs: processing {len(micro_batches)} micro batches...")
    ...
    def update_policy(self, data: DataProto) -> dict[str, Any]:
        ...
        for _ in range(self.config.ppo_epochs):
            if self.rank == 0:
                ...
                print(f"Train mini-batches: processing {len(mini_batches)} mini batches...")
            ...
            for i, mini_batch in enumerate(mini_batches):
                ...
                if self.rank == 0:
                    ...
                    print(f"    [MiniBatch {i+1}/{len(mini_batches)}] Updating policy with {len(micro_batches)} micro-batches")
                    ...
                    if self.config.use_kl_loss and "ref_log_probs" in model_inputs:
                        ...
                        if getattr(self.config, "dynamic_kl_loss", True) and ("difficulty" in model_inputs):
                            ...
                            with torch.no_grad():
                                ...
                                seq_mean_kl = (kld * mask).sum(dim=1) / valid
                                self._update_beta_per_bucket(difficulty_levels, seq_mean_kl)
                            beta_per_seq = self._beta_for_levels(difficulty_levels, device=kld.device)
                            kld = kld * beta_per_seq
                        else:
                            kld = kld
