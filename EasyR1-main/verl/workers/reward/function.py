...
class BatchFunctionRewardManagerMixin:
    ...
    def compute_reward_batch(self, data: DataProto) -> Tuple[torch.Tensor, dict[str, list[float]]]:
        ...
        for i in range(len(data)):
            ...
            #     }
            # )
            difficulties.append(data.non_tensor_batch.get("difficulty", ["easy"] * len(data))[i])

        bootstrap_accs = data.non_tensor_batch.get("bootstrap_acc", [0.0] * len(data)).tolist() if hasattr(data.non_tensor_batch.get("bootstrap_acc", [0.0] * len(data)), 'tolist') else list(data.non_tensor_batch.get("bootstrap_acc", [0.0] * len(data)))
        scores = self.reward_fn(response_str, data.non_tensor_batch['correctness'], difficulties=difficulties, bootstrap_accs=bootstrap_accs, resp_lengths=response_length) #, ref_resp_lengths=data.non_tensor_batch['ref_resp_lengths'])
        reward_tensor = torch.zeros_like(data.batch["responses"], dtype=torch.float32)
        reward_metrics = defaultdict(list)
