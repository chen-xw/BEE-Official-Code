...
from examples.reward_function.eva_reward_adaptive import rule_then_api_batch_judge
from examples.reward_function.eva_reward_adaptive import extract_and_check as easyr1_monet_extract_and_check
from examples.reward_function.eva_reward_adaptive import extract_and_check_api as easyr1_monet_extract_and_check_api
...
class FSDPWorker(Worker):
    def __init__(
        ...
        if self._is_rollout and "api" in self.config.rule_based_judge.judge_function_name:
            ...
            elif self.config.rule_based_judge.api_name in ['Qwen2.5-VL-32B', 'Qwen3-VL-235B-A22B-Instruct', 'Qwen3.5-397B-A17B', 'Qwen3-VL-4B']:
                self.api_client = build_qwen_client()
            else:
    ...
    def compute_rule_based_judge(self, data: DataProto):
        ...
        if self.config.rule_based_judge.judge_function_name=="rule_then_api_batch_judge":
            ...
            for i in tqdm(range(len(data)), desc="Rule-based judge", position=position, disable=self.rank != 0):
                ...
                if self.config.rule_based_judge.judge_function == "./examples/reward_function/eva_reward_adaptive.py:extract_and_check":
                    correctness.append(easyr1_monet_extract_and_check(response_str, ground_truth))
                elif self.config.rule_based_judge.judge_function == "./examples/reward_function/eva_reward_adaptive.py:extract_and_check_api":
