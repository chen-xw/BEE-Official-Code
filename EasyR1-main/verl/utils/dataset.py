...
from PIL import Image, PngImagePlugin # 确保导入了 PngImagePlugin

# 将限制扩大到 100MB (或者更大)，默认是 1MB
...
class RLHFDataset(Dataset):
    ...
    def _build_messages(self, example: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if self.image_key in example:
            # [关键修复] 获取实际存在的图片数量
            actual_images = example.get(self.image_key, [])
            num_images = len(actual_images)
            ...
            if "<image>" in prompt_str:
                ...
                for i, content in enumerate(parts):
                    # 只有当我们还有剩余图片配额时，才添加 image 占位符
                    if i != 0:
                        if image_counter < num_images:
                            content_list.append({"type": "image"})
                            image_counter += 1
                        else:
                            # 图片用完了，文本里多余的 <image> 标签直接被忽略
                            pass 

                    if content:
                        content_list.append({"type": "text", "text": content})
            else:
                # 兜底逻辑：如果文本没标签但有图，把图全加上
    ...
    def __getitem__(self, index):
        ...
        if 'id' in example:
            example['global_id'] = example.get("id", f"example-{index:06d}")
        return example
